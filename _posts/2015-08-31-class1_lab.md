...
## Before going any further, if you don't have one already, go to Github.com, sign-up, and download GitHub Desktop.  Next, go to the [repositiory for the course](https://github.com/RPI-Analytics/MGMT6963-2015) and *fork* the repository using the button on the top right.  

*When using Git, start with GitHub Desktop but you should also learn your way around the command line.  You will likely find it useful if you have to make changes directly on a unix machine.*
...
For our Python and R work, we will be using a custom virtual machine.  We could do the same thing and distribute a virtual machine.  However, every time we wanted to make an adjustment to the virtual machine we would have to re-download the entire machine (~5 GB). Not cool!  Instead we will be using som nifty tools to be able to configure a virtual machine from a "base box".
...
***Insert picture ***
When you first ssh into your guest machine.  You will be in your home directory, which is the directory that linux has partitioned for your works.  In the case of vagrant configured machines, the user is actually called `vagrant` and the directory is `\home\vagrant`.  Wherever you are in the directory structure, you can always type `cd ` and you will return home.  

Files on your *host* machine in the repository directory will be mapped to the `/vagrant` directory on the guest machine.  
